...
> Status convention: each task line includes `*(Done)*`, `*(In progress)*`, or `*(Not done)*` based on the repo audit (2026-09-18, post PR #13). Epic 9 closed the pre-#6 docs wave; Epic 12 tracks remaining installer/uninstaller drift (including the #10 Cursor dest move to `.cursor/commands/`, plus MCP `WORKSPACE_ROOT` not pinned by installers). Catalog still matches the tree (24 templates / 13 DoD). This pass adds Task 8.6 (MCP pytest not in GitHub Actions) and Task 12.8 (`WORKSPACE_ROOT`).
...
> Goal: Meet all automated testing and static validation requirements outlined in the product specification (Section 6). MCP unit tests exist locally but are not a GitHub Actions job (Task 8.6).
