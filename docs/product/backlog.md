...
- **Task 1.4**: Add agent and rule support to all three installers (i.e., also iterate over `agents/*/` and `rules/*/`).
...
- **Task 2.3**: Author `commit-message` rule — passive format guideline for conventional commits. Author for `cursor`, `ghcp`, `claude`.
