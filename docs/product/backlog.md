...
> Status convention: each task line includes `*(Done)*`, `*(In progress)*`, or `*(Not done)*` based on the repo audit (2026-09-18, post PR #13). Epic 9 closed the pre-#6 docs wave; Epic 12 tracks remaining installer/uninstaller drift (including the #10 Cursor dest move to `.cursor/commands/`). Catalog tests still match the tree (24 templates / 13 DoD). New this pass: Task 8.6 (`run_tests.sh` reports success when required suites did not run).
...
> Goal: Bring README, product docs, and technical design docs back in sync with the actual repository layout and behaviour (post `install/` move, Cursor support, MCP, tests). Re-synced 2026-09-16 for PR #12 (`sdlc-researcher` / `sdlc-setup-repository` + templates/DoD; catalog tests 24/13). 2026-09-18 pass aligned leftover "authored once per harness / no Python for install" claims with `CONTENT.md` expansion and the live-install `python3` MCP merge. Remaining code-level installer drift is Epic 12.
...
- **Task 10.7** *(Done)*: Minimal discoverability docs — architecture catalog, README pointer, installation driver list. *(Evidence: `docs/technical_design/architecture.md`, `README.md`, `docs/guides/installation-and-usage.md`. 2026-09-16 review added `research` / `repository setup` / `sdlc-researcher` / `sdlc-setup-repository` pointers.)*
...
- **Task 10.9** *(Not done)*: Close leftover MCP alias/regression coverage after PR #12. Catalog counts now match, but `test_get_sdlc_template_new_names_and_aliases` still has no cases for `product spec` / `prd`, `research` / `spike` / `investigation`, or `repository setup`. `test_get_definition_of_done_new_names_and_aliases` likewise omits `research` and `repository setup`. `ALIASES` has no phrases for repository setup. Add those parametrized tests, add reasonable `repository setup` aliases, and add a regression that `get_sdlc_template("epic")` returns the product-spec template while `get_definition_of_done("epic")` stays Epic DoD (catalogs are separate; exact DoD match already exists).
...
> Goal: Close the code and test gaps left after PR #6 (MCP merge, venv Python, uninstallers) and PR #10 (Cursor dest `.cursor/commands/`). Docs were re-synced on 2026-09-16; these items are remaining implementation work.
