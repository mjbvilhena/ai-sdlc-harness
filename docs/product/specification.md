...
2. The harness invokes the native Lifecycle Driver prompt (today: a skill under `skills/`). The default front door is `sdlc-conductor` (`docs/technical_design/sdlc-conductor.md`); specialists such as `sdlc-code-reviewer` remain directly invokable. Dedicated `agents/` packages are deferred.
...
- Installers MUST NOT require any runtime dependency beyond standard Unix utilities (`bash`, `cp`, `mkdir`, `cat`, `grep`, `mktemp`) or native PowerShell **for the copy/expand step**. A live (non-`--dry-run`) install currently invokes host `python3` to merge MCP JSON and fails under `set -e` if that interpreter is missing (Task 12.5).
...
- **Zero dependencies for copy/expand**: The skill/rule copy step requires no external runtimes like Node. A live installer currently needs host `python3` only to merge MCP JSON (Task 12.5); running the knowledge server needs the `mcp-server/venv` Python.
