        ...
        → per-epic user-story refine (draft)
          → story approval (human)
            → design & planning band (per epic or per release slice — may parallelize where legal):
                 ...
                 • test strategy (`sdlc-test-planner` — test plan + e2e plan; `sdlc-threat-modeler` when the slice warrants)
...
| **Test strategy** | How the slice will be proven, written **before** implementation tests are generated | `sdlc-test-planner`. MCP templates `test_plan`, `e2e_test_plan`. `sdlc-threat-modeler` (template `threat_model`) when the slice is security-sensitive. `sdlc-security-reviewer` is **verify-time** (control checklist on the change). `sdlc-test-writer` / `sdlc-e2e-scripter` are **later execution**, not this state’s authors | Test plan (and e2e plan when there is a user journey) exists and is linked; threat-model artefact exists when warranted |
...
| **Implement** | Build the story against **approved** design + AC | `sdlc-implementer`. Meet `user story` + `feature` DoD and change-type DoD (`ui change`, `api change`, `security change`) when those apply. Not `sdlc-test-writer`. | Planning artefacts above are **approved** (or an explicit skip/treat-as-approved was recorded). Must AC in progress; no silent extra features. Done only when automation is green **and** all Must AC are met; unmet Must AC → new stories, no soft-pass |
| **Verify** | Prove the increment; may stay **adjacent** to implement | `sdlc-test-writer`, `sdlc-e2e-scripter` (execute the strategy), `sdlc-pr-summarizer` (PR packaging before or with review), `sdlc-code-reviewer`, `sdlc-security-reviewer` (control checklist), `sdlc-dod-checker`, `sdlc-a11y-auditor` for UI | **Done** only when test automation passes without errors **and** all Must AC are fully met. If some Must AC cannot be met: do **not** soft-pass — create additional user stories. Human review for consequential merges |
| **Later / ops drivers** | Ship, operate, recover, **or defect intake** — **not** a dumping ground for design | `sdlc-bug-triager` (intake → diagnosable ticket → stories/fix; not the setup→tests happy path), `sdlc-release-notes-generator`, `sdlc-runbook-writer`, `sdlc-migration-planner`, `sdlc-postmortem-writer`, `sdlc-ci-debugger`, … | Invoked when the **current** ops/ship/intake need matches |
...
| **Test strategy vs execution** | `sdlc-test-planner` authors MCP `test_plan` / `e2e_test_plan` (and `sdlc-threat-modeler` / `threat_model` when the slice is security-sensitive) **before** code. `sdlc-test-writer` / `sdlc-e2e-scripter` execute that plan at **verify** time. `sdlc-security-reviewer` is the verify-time control checklist (complements design-time STRIDE; it is not the test-strategy author). |
...
| **`sdlc-implementer`** | Child **after** planning + setup. Builds approved stories against story/feature (+ change-type) DoD. Does not invent scope. Story Done = automation green **and** all Must AC met; unmet Must AC → new stories, no soft-pass. Not a substitute for test strategy or verify. |
...
| **Verify-adjacent** (`sdlc-test-writer`, `sdlc-e2e-scripter`, `sdlc-pr-summarizer`, `sdlc-code-reviewer`, `sdlc-security-reviewer`, `sdlc-dod-checker`, `sdlc-a11y-auditor`) | Adjacent to **implement**, not a substitute for test strategy or UX design. PR summarizer packages the change before or with `sdlc-code-reviewer`. Security-reviewer is verify-time. A11y is verify-time. |
| **Later / ops** (`sdlc-bug-triager`, `sdlc-release-notes-generator`, `sdlc-runbook-writer`, `sdlc-migration-planner`, `sdlc-postmortem-writer`, `sdlc-ci-debugger`, …) | After a ship/ops/intake need appears. `sdlc-bug-triager` is the defect-intake path into triage → stories/fix — **not** an edge on the setup→tests happy path. Not where RFC, API design, or test strategy live. |
...
- **Not replacing child authorship.** Spec, stories, RFC / API / test-plan / UX bodies, setup instructions, implementation, DOMAIN/LAYER bodies, ADRs, and review PRs stay with their skills (`sdlc-test-planner` for strategy, `sdlc-implementer` for code).
...
- The **pipeline graph** (states, legal edges, skip rule, discovery table) lives in MCP template `lifecycle pipeline`. Skill `CONTENT.md` stays thin and must fetch that payload before executing. Do not fork a second informal list in README, and do not collapse the graph to stories → setup → test-writer.
- UI/UX hand-off is `sdlc-ux-designer` (Task 13.6). Fetch MCP template `ux design` (aliases `ux`, `wireframe`).
- Test-strategy hand-off is `sdlc-test-planner` (Task 13.9). Implementation hand-off is `sdlc-implementer` (Task 13.10).
...
9. **Who authors the test-strategy doc** — **Resolved:** `sdlc-test-planner` authors `test_plan` / `e2e_test_plan` in the design/planning band. `sdlc-test-writer` / `sdlc-e2e-scripter` remain verify-time execution. Do not treat those execution skills as the strategy author. Runtime copy lives in MCP template `lifecycle pipeline`.
