        ...
        → per-epic user-story refine (draft)
          → story approval (human)
            → design & planning band (per epic or per release slice — may parallelize where legal):
                 ...
                 • test strategy (test plan + e2e plan; `sdlc-threat-modeler` when the slice warrants)
...
| **Test strategy needed** | Slice has stories but no test plan (and no e2e plan when a journey exists). Security-sensitive slice with no threat model → hand off to `sdlc-threat-modeler`. |
