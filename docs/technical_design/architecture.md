...
A planned default front door, **`sdlc-conductor`**, is specified in [`sdlc-conductor.md`](sdlc-conductor.md) (Epic 13). It is **not** in `skills/` yet. When shipped it should recommend the next legal pipeline step and steward cross-cutting memory; it must not replace child-skill authorship or `sdlc-docs-backlog-review`.
