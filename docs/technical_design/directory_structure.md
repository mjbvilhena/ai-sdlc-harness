...
- The `skills/` and `agents/` directories are the only runtime source of truth. The installer scripts read from these directories and nothing else.
- `skill.yaml` / `agent.yaml` are for human readers, documentation generators, and CI validators. Installer shell scripts do not parse them.
- Target-specific subdirectories (`agy/`, `claude/`, `ghcp/`) are optional per skill. A skill that only targets AGY only needs an `agy/` subdirectory.
