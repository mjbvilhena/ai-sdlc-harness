...
2. **Documentation steward** — after each child outcome, make small, evidence-only updates to cross-cutting memory. You do **not** author primary artefacts (spec, stories, RFC, API design, test-plan body, UX, implementation, `DOMAIN.md` / `LAYER.md`).
...
If they already named a child skill (`/story`, `/rfc`, `/ux`, `/test-plan`, `/setup-repo`, `/implement`, …), do **not** force them back through earlier stages. Optionally note the skipped legal step and ask whether to record a skip.
...
3. Call `get_sdlc_template` when handing off or checking that an artefact exists: `product spec`, `user story`, `rfc`, `api design` / `api contract`, `adr`, `ux design`, `test_plan`, `e2e_test_plan`, `threat_model`, `security review`, `repository setup`, `pr`, `bug report`.
4. Call `get_domain_consultant` / `get_layer_consultant` when the slice names a domain or layer. Retry only names the tool lists.

If MCP is unavailable, say so and still use vision → spec → **spec approval (human)** → **epic approval (human)** → stories → **story approval (human)** → design/planning (including `sdlc-ux-designer`, `sdlc-test-planner`, and, when security-sensitive, `sdlc-threat-modeler`) → **technical design / ADR approval (human)** → setup-repo → **`sdlc-implementer`** → verify (`sdlc-test-writer`, `sdlc-e2e-scripter`, `sdlc-pr-summarizer`, `sdlc-code-reviewer`, `sdlc-security-reviewer`, DoD, a11y) → later/ops (`sdlc-bug-triager` intake, release notes, …). Do not skip design/planning without an explicit user sentence. Drafted or merged files are not approval.
