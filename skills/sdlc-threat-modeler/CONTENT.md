...
1. **CRITICAL**: Call `get_sdlc_template` with `template_type="threat model"` (alias `stride` also resolve). Follow that document's sections, STRIDE categories, finding table, and quality bar.
...
2. Walk each STRIDE category from the fetched template that is relevant to the change. Skip the rest with "not applicable" and a reason.
...
4. Call out missing controls from the template only when the change actually involves that surface.
