...
@test "install_cursor.sh workspace install is idempotent" {
    ...
    cmp -s "$expected" "$MOCK_WORKSPACE/.cursor/commands/sdlc-code-reviewer.md"
}
...
@test "install_cursor.sh removes stale sdlc-* rules and keeps neighbors" {
    mkdir -p "$MOCK_WORKSPACE/.cursor/commands"
    printf '%s\n' 'stale' > "$MOCK_WORKSPACE/.cursor/commands/sdlc-stale-skill.md"
    printf '%s\n' 'keep' > "$MOCK_WORKSPACE/.cursor/commands/my-custom-rule.md"
    run "$REPO_ROOT/install/install_cursor.sh" --workspace "$MOCK_WORKSPACE"
    [ "$status" -eq 0 ]
    [ ! -f "$MOCK_WORKSPACE/.cursor/commands/sdlc-stale-skill.md" ]
    [ -f "$MOCK_WORKSPACE/.cursor/commands/my-custom-rule.md" ]
    [ -f "$MOCK_WORKSPACE/.cursor/commands/sdlc-code-reviewer.md" ]
    [ -f "$MOCK_WORKSPACE/.cursor/commands/sdlc-dod-checker.md" ]
}
